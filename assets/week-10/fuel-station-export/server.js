#!/usr/bin/env node
// ============================================================
// AI Fuel Station — real-time data server
//
//   node server.js                # http://localhost:8753
//
// Fetches LIVE data on the server (no browser CORS) and serves it to the
// front-end at /api/fuel-data. The page calls that endpoint on load, so the
// pumps always reflect the latest catalogue.
//
// DATA SOURCES (all live):
//   • api.llm-stats.com/stats/v1/models   (the keyed public API)
//       -> model id · name · organization · input/output price · benchmark scores (IQ)
//   • llm-stats.com/models  +  per-model detail pages
//       -> throughput  (REAL output speed, tokens/sec) joined by model id
//
//   IQ is computed from the benchmark scores (see scoreToIQ).
//   Speed is REAL where llm-stats publishes throughput; the few models with no
//   published telemetry fall back to a price-tier estimate (flagged isEstSpeed).
// ============================================================
...
const REFRESH_MS = 30 * 60 * 1000; // re-pull every 30 min
...
const BRAND = {
  ...
  minimax: ['#e2614a', '#9c3324'], moonshot: ['#5a4ad1', '#332a8f'], zhipu: ['#22a39f', '#13615e'],
...
async function buildData() {
  ...
  // website catalogue → per-model benchmark scores (for IQ) + coarse throughput
  ...
  const { byKey, benchDist } = webCat;
  const catDist = buildCatDist(models);
  const webRec = (m) => byKey[norm(m.id)] || byKey[norm(m.name)] || null;
  // IQ: pool every measured axis (hard benchmarks + capability categories) as a
  // midpoint-percentile across all models, then shrink toward the median by coverage.
  // → fair across difficulties, and thin-data models can't top the chart on luck.
  const iqFor = (m) => {
    const rec = webRec(m);
    const ps = [].concat(
      rec ? collectPercentiles(rec.scores, benchDist, IQ_BENCH) : [],
      collectPercentiles(m.top_scores, catDist, IQ_CATS)
    );
    return iqFromPercentiles(ps);
  };

  // keep real, scoreable text LLMs only
  ...
  // de-dupe families + sort, then build stations sorted by line-up size
  let stations = Object.values(orgs).map((o, idx) => {
    ...
    const models = Object.values(byFam).sort((a, b) => b.iq - a.iq);
    return { ...o, models, color: colorFor(o.id, idx) };
  }).filter((o) => o.models.length > 0)
    .sort((a, b) => b.models.length - a.models.length || b.models[0].iq - a.models[0].iq);
...
async function refresh() {
  try { cache = await buildData(); console.log(`[data] ${cache.modelCount} models · ${cache.stationCount} stations @ ${cache.updatedAt}`); }
  catch (e) { console.error('[data] refresh failed:', e.message); }
}
function ensureData() { if (!building) building = refresh().finally(() => { building = null; }); return building; }
...
const server = http.createServer(async (req, res) => {
  ...
  if (url === '/api/fuel-data') {
    if (!cache) await ensureData();
    res.writeHead(200, { 'content-type': 'application/json', 'access-control-allow-origin': '*', 'cache-control': 'no-store' });
    return res.end(JSON.stringify(cache || { live: false, stations: [] }));
  }
  if (url === '/api/refresh') { await refresh(); res.writeHead(200, { 'content-type': 'application/json' }); return res.end(JSON.stringify({ ok: true, updatedAt: cache && cache.updatedAt })); }
  // static
  let file = url === '/' ? '/fuel-station.html' : url;
...
server.listen(PORT, () => {
  console.log(`AI Fuel Station → http://localhost:${PORT}`);
  refresh();
